...
struct Mat4
{
    ...
    Mat4 Rotate(const Vec3& eulerRotation) const;
...
inline Mat4 Mat4::Orth(Float left, Float right, Float bottom, Float top, Float zNear, Float zFar)
{
    Mat4 t{ identity };
...
inline Mat4 Mat4::Perspective(Float verticalFov, Float aspectRatio, Float zNear, Float zFar)
{
    Mat4 t{ identity };

    Float tanHalfFov = std::tan(verticalFov / 2);

    // Scale
    t.ex.x = 1 / (aspectRatio * tanHalfFov);
    t.ey.y = 1 / tanHalfFov;
    t.ez.z = -(zFar + zNear) / (zFar - zNear);
    t.ez.w = -1; // Needed for perspective division

    // Translation (for z-axis)
    t.ew.z = -(2 * zFar * zNear) / (zFar - zNear);
...
inline Mat4 Mat4::LookAt(const Vec3& position, const Vec3& target, const Vec3& up)
{
